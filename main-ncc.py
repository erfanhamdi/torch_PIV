...
for j in tqdm(range(jm)):
    ...
    for i in range(im):
        ...
        R_torch = torch_corr(c1, img_2[sw_l:sw_r,sw_d:sw_u])
        ...
        if rij[i,j]>=r_limit:
            ...
            vecy[i, j] = center_of_max_R_in_image_2[1] - center_pixel_c1_in_image_1[1] + subpix(R_torch[0], 'y', dum)
            vecx[i, j] = center_of_max_R_in_image_2[0] - center_pixel_c1_in_image_1[0] + subpix(R_torch[0], 'x', dum)
